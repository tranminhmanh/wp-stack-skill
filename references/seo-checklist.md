...
## Robots.txt: physical file at docroot overrides WP virtual + Rank Math hooks

**Symptom**: site has `Sitemap:` declarations in `robots.txt` that do NOT match what Rank Math should emit. Some URLs return 404. WP `do_robots()` filter + Rank Math sitemap hook do not appear to fire — `robots.txt` is "stuck" on stale content from migration.

**Root cause**: a physical file at `<docroot>/robots.txt` exists (often from a migration / template builder). Apache serves the physical file before PHP runs → WordPress's virtual `robots.txt` (generated by `do_robots()` action + Rank Math hook) never reaches the response.

**Detection** — distinguish virtual vs physical:
```bash
curl -sI "https://<site>/robots.txt"
```

Indicators of a **physical file** (not WP virtual):
- `last-modified: <date in past>` → virtual would show current timestamp
- `accept-ranges: bytes` → file-system serving
- Content has non-WP formatting (e.g. `Sitemap:URL` no space after colon, manually-typed feel)
- Server cache headers treat it like static (LiteSpeed pragma cache)

WP virtual `robots.txt`:
- NO `last-modified` header (dynamic)
- NO `accept-ranges`
- Content matches Rank Math's expected output

**3 fix paths**:

1. **Edit the physical file** via cPanel → File Manager → `public_html/robots.txt`. Quick, low-risk. Best when you want explicit control.
2. **Delete the physical file**. Apache falls through to WP → `do_robots()` action runs → Rank Math hook auto-emits `sitemap_index.xml`. Best for clean Rank Math integration.
3. **`.htaccess` rewrite** (advanced): `RewriteRule ^robots\.txt$ /index.php?robots=1 [L]`. Forces every request through WP. Risk: nginx-served sites don't honor `.htaccess`.

**Audit checklist when inheriting a site**:
```bash
# Spot a physical robots.txt
curl -sI "https://<site>/robots.txt" | grep -iE 'last-modified|accept-ranges'

# Count sitemap declarations (often inflated by migration)
curl -s "https://<site>/robots.txt" | grep -c '^Sitemap:'

# HEAD-check every declared sitemap — many 404 = migration debris
curl -s "https://<site>/robots.txt" | grep -oP '^Sitemap:\s*\K\S+' \
  | while read url; do
      printf '%s  ' "$(curl -sI "$url" | head -1)"
      echo "$url"
    done
```

A real audit found `robots.txt` declaring 21 sitemaps with 20 returning 404 (template leftovers). Cleanup = 1 canonical `sitemap_index.xml` line.

**Reusability**: universal — common gotcha on migrated WordPress sites.

## Schema graph `@id` linking — link entities, don't duplicate them

When a site has multiple schema entities (Organization, MedicalClinic, Physician, MedicalProcedure / Service / Article), use `@id` URL fragments to **link** entities across pages instead of duplicating the full property set in every page's schema.

**Why this matters**:
- Knowledge-graph signal: Google understands entity hierarchy + relationships
- Payload size: no duplicate Organization JSON on every service page
- Maintainability: change Organization once at the root → every reference inherits
- Schema.org validator passes cleanly with referenced entities

**Pattern — root entity at homepage, references everywhere else**:

```jsonld
// 1. ROOT entity on homepage — fragment #organization
{
  "@type": "MedicalClinic",
...
// 2. SERVICE page references back via @id (no duplicate Organization data)
{
  "@type": "MedicalProcedure",
  "@id": "https://example.com/service-a/#procedure",
  "name": "Service A",
  "performer": { "@id": "https://example.com/#organization" },
  "isPartOf":  { "@id": "https://example.com/service-a/#webpage" }
}

// 3. PERSON page references the same Organization
{
  "@type": "Physician",
  "@id": "https://example.com/staff/dr-x/#physician",
  "name": "Dr X",
  "worksFor": { "@id": "https://example.com/#organization" }
}

// 4. HUB page builds a CollectionPage with ItemList of references
{
  "@type": "CollectionPage",
    ...
    "itemListElement": [
      {
        ...
        "item": { "@id": "https://example.com/service-a/#procedure" }
      }
    ]
  }
}
...
**Naming convention for `@id` fragments** (pick one and stick to it):
- `#organization` — root entity (Organization / LocalBusiness / MedicalClinic)
- `#webpage` — current page's WebPage entity
- `#breadcrumb` — BreadcrumbList for the page
- `#procedure` / `#service` / `#test` — service-specific node
- `#physician` / `#person` — Person / Physician
- `#article` — Article node (note: Rank Math default uses `#richSnippet` — pick your convention OR match Rank Math's)

**Anti-pattern**: duplicating the full Organization properties (address, phone, name, sameAs) on every service page's JSON-LD. Wastes bytes + risks data drift (one page shows old phone number).

**Reusability**: universal for any structured data implementation on a multi-page site.
